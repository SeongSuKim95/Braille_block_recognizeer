...
class SurfaceSegmentation(Dataset):
    ...
    def __init__(self, base_dir=settings.root_dir, split='train'): ## base_dir = settings.root_dir
        super().__init__()
        self.split = split
        self.images = []
        self.masks = []
        file2_list = Path(base_dir)
        ...
        if split == 'train':
            self.composed_transform = transforms.Compose([
                ...
                tr.ToTensor()])
            # self.composed_transform =transforms.Compose([transforms.ToTensor(),
