...
#Tensorboard = ./PycharmProjects/DP/segmentation-selectstar-master/run/surface/deeplab
#tensorboard --logdir=./PycharmProjects/DP/segmentation-selectstar-master/run/surface/deeplab --host=0.0.0.0

class Trainer(object):
    def __init__(self,):
        # Define Saver
        self.saver = Saver()
        self.saver.save_experiment_config() ## setting.py file copy

        # Define Tensorboard Summary
        self.summary = TensorboardSummary(self.saver.experiment_dir) ## TensorboardSummary == modules.utils->>summarywriter
        self.writer = self.summary.create_summary()
        ## ==SummaryWriter(./run/datasetname/checkpointname/experiment_*)
        ...
        # Define lr scheduler
        self.scheduler = LR_Scheduler(settings.lr_scheduler, settings.lr,settings.epochs, len(self.train_loader))
    ...
    def training(self, epoch):
        ...
        for i, sample in enumerate(train):
            ...
            # concat=cv2.hconcat([image,target])
            # cv2.imshow('d',concat)
            # cv2.waitKey(0)
            if settings.cuda:
            ...
            if i % 10 == 0:
                print('Batch Size: %d,Num_img_tr : %d, Epoch : %d, iteration: %d, Train loss: %.3f' % (settings.batch_size,num_img_tr,epoch, i, (train_loss/(i+1))))
            ##Tensorboard ->> loss, total_iter , iter + batchsize * epoch
    ...
    def validation(self, epoch):
        ...
        for i, sample in enumerate(validation):
            ...
            print('Batch Size: %d,Num_img_tr : %d, Epoch : %d, iteration: %d, Train loss: %.3f' % (settings.test_batch_size, len(validation), epoch, i, (test_loss / (i + 1))))
